...
def save_output_as_pickle(results, out_path):
    ...
    for result_d in tqdm(results, unit="result"):
        ...
        flow_results_rec.append(
            result_d["flow_results"].cpu().numpy()
        )
        num_results += 1

    err_msg = "Mismatch in no. of samples."
    assert num_results == len(results), err_msg

    # Create a new dictionary with lists of tensors.
    out_dict = {
        "occ_results": occ_results_rec, 
        "flow_results": flow_results_rec,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "without perform evaluation. It is " + \
        "useful when you want to format the " + \
        "result to a specific format and " + \
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="evaluation metrics, which " + \
        "depends on the dataset, e.g., 'bbox', " + \
        "'segm', 'proposal' for COCO, and 'mAP', " + \
    ...
    parser.add_argument(
        "--tmpdir",
        help="tmp directory used for collecting " + \
        "results from multiple workers, available " + \
        "when gpu-collect is not specified"
    )
    parser.add_argument(
        "--seed", 
        type=int, 
    ...
    parser.add_argument(
        "--deterministic",
        action="store_true",
        help="whether to set deterministic " + \
        "options for CUDNN backend."
    )
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
        help="override some settings in the used " + \
        "config, the key-value pair in xxx=yyy " + \
        "format will be merged into config file. " + \
        "If the value to be overwritten is a list, " + \
        "it should be like key='[a,b]' or key=a,b " + \
        "It also allows nested list/tuple values, " + \
        "e.g. key='[(a,b),(c,d)]' " + \
    ...
    parser.add_argument(
        ...
        nargs="+",
        action=DictAction,
        help="custom options for evaluation, the " + \
        "key-value pair in xxx=yyy format will be " + \
        "kwargs for dataset.evaluate() function " + \
        "(deprecate), change to --eval-options instead."
    )
    parser.add_argument(
        "--eval-options",
        nargs="+",
    ...
    if args.options and args.eval_options:
        raise ValueError(
            "--options and --eval-options cannot " + \
            "be both specified, --options is deprecated " + \
            "in favor of --eval-options"
        )
    if args.options:
        warnings.warn(
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            for key in [
                ...
                "save_best", "rule", "begin", "end"
            ]:
                eval_kwargs.pop(key, None)
            eval_kwargs.update(dict(metric=args.eval, **kwargs))

            dataset.evaluate_miou(
                occ_results=outputs, 
